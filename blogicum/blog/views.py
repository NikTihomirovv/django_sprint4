...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if post.author == self.request.user:
            ...
            Post.objects.all().filter(Q(is_published=True) | (Q(is_published=False) & Q(author=self.request.user))),
                pk=self.kwargs.get('pk'))
        else:
            context['page_obj'] = get_object_or_404(Post.objects.select_related(
            'category',
            'author',
            'location').filter(is_published=True,
                               category__is_published = True,
